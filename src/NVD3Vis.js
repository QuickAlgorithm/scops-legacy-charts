...
const chartSteps = Object.freeze({
  //the step 0 is a placeholder and represent the inital state of the chart
...
function nvd3Vis(element, props) {
  ...
  const onBarClick = barData => {
    const stepsArray = Object.keys(chartSteps);
    const maxStep = stepsArray[stepsArray.length - 1];
    if (window.__scopsActualChartStep + 1 > maxStep) return;
    window.__scopsActualChartStep += 1;
    if (window.__scopsActualChartStep === 0) return;

    const date = moment(barData.x);
    const stepConfig = chartSteps[window.__scopsActualChartStep];

    const startRange = date
      .clone()
      .startOf(stepConfig.timeBound)
    ...
    window.__scopsChartStepHystory.push({ timeRange, granularity: stepConfig.granularity, label: label });
  };
  ...
  const drawGraph = function drawGraph() {
    ...
    const applyYAxisBounds = () => {
      if (chart.yDomain && Array.isArray(yAxisBounds) && yAxisBounds.length === 2) {
        ...
        if ((hasCustomMin || hasCustomMax) && vizType === 'area' && chart.style() === 'expand') {
          ...
          if (
            vizType === 'area' ||
            (isVizTypes(['bar', 'dist_bar', 'scops_bar']) && chart.stacked())
          ) {
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      if (isTimeSeries && activeAnnotationLayers.length > 0) {
        ...
        let xMin;
        let xScale;
        if (vizType === 'bar' || vizType === 'scops_bar') {
